...
/**
 * Manager Description Config that defines the manager description which contains the cube dimensions and every
 * CopperMeasure.
 *
 * @author ActiveViam
 */
...
public class ManagerDescriptionConfig implements IActivePivotManagerDescriptionConfig {
	...
	/**
	 * Measure for the ratio of off-heap memory relative to the application's total memory consumption.
	 */
	 ...
	 * <p>
	 * Not equal to {@code contributors.COUNT} since MAC's base store granularity is finer than the granularity of
	 * the chunk.
	 */
	 ...
	 * <p>
	 * This measure only sums the on-heap memory held by chunk, and therefore do not contains the entire on-heap
	 * footprint of the entire ActivePivot Application
	 */
	...
	/**
	 * Measure of the the non-written rows in Chunks, relative to the total non-written rows in the application.
	 */
	...
	private ISelectionDescription memorySelection() {
		return StartBuilding.selection(datastoreDescriptionConfig.datastoreSchemaDescription().asDatabaseSchema())
				...
				.withAllReachableFields(allReachableFields -> {
					...
					final Map<String, FieldPath> result =
							ISelectionDescriptionBuilder.FieldsCollisionHandler.CLOSEST.handle(allReachableFields);
					result.put(
	...
	private IActivePivotInstanceDescription memoryCube() {
		return StartBuilding.cube(MONITORING_CUBE)
				...
				.withSharedContextValue(QueriesTimeLimit.of(TIMEOUT_DURATION.getSeconds(), TimeUnit.SECONDS))
	...
	private ICanBuildCubeDescription<IActivePivotInstanceDescription> defineDimensions(
		...
		return builder.withDimension(CHUNK_DIMENSION)
				...
				.withHierarchyProperty(
						"description",
						"The ID of the dictionary the chunk is attributed to, if any")
				.withLevelOfSameName()
				.withPropertyName(DatastoreConstants.CHUNK__PARENT_DICO_ID)
				.withLevelProperty("description", "The ID of the dictionary the chunk is attributed to, if any")
				.withHierarchy(CHUNK_INDEX_ID_LEVEL)
				.withHierarchyProperty("description", "The ID of the index the chunk is attributed to, if any")
				.withLevelOfSameName()
				...
				.withHierarchyProperty("description", "The ID of the reference the chunk is attributed to, if any")
				.withLevelOfSameName()
				.withPropertyName(DatastoreConstants.CHUNK__PARENT_REF_ID)
				.withLevelProperty("description", "The ID of the reference the chunk is attributed to, if any")
				...
				.withDimensionProperty("description", "The ID of the store or cube partition that holds the chunk")
				.withHierarchy(PARTITION_HIERARCHY)
				.withLevelOfSameName()
				.withLevelProperty("description", "The ID of the store or cube partition that holds the chunk")
				...
				.withHierarchyProperty(
						"description",
						"The source folder name from which the statistics were retrieved")
				.withLevelOfSameName()
				...
				.withDimensionProperty("description", "Holds hierarchies relative to pivot aggregate providers")
				.withHierarchy(MANAGER_HIERARCHY)
				.withHierarchyProperty("description", "The id of the manager associated with the chunk, if any")
				.withLevelOfSameName()
				...
				.withDimensionProperty("description", "Holds hierarchies relative to the versioning of the chunks")
	...
	private void joinViewVersion(ICopperContext context) {
		final CopperStore epochViewStore = Copper.store(DatastoreConstants.EPOCH_VIEW_STORE)
				.joinToCube()
	...
	private void chunkMeasures(final ICopperContext context) {
		perChunkAggregation(Copper.constant(1L)).sum()
				.as(CHUNK_COUNT)
				.withinFolder(CHUNK_FOLDER)
				.withDescription("the number of contributing chunks")
				.publish(context);

		perChunkAggregation(DatastoreConstants.CHUNK__ON_HEAP_SIZE).sum()
				.as(HEAP_MEMORY_SUM)
				.withFormatter(ByteFormatter.KEY)
		...
		final CopperMeasure chunkSize = perChunkAggregation(DatastoreConstants.CHUNK__SIZE).sum()
				.as(CHUNK_SIZE_SUM)
				.withFormatter(NUMBER_FORMATTER)
				.withinFolder(CHUNK_FOLDER)
		...
		final CopperMeasure nonWrittenRowsCount =
				perChunkAggregation(DatastoreConstants.CHUNK__NON_WRITTEN_ROWS).sum()
		...
		perChunkAggregation(DatastoreConstants.CHUNK__FREE_ROWS).sum()
		...
		nonWrittenRowsCount.divide(chunkSize)
				.withType(Types.TYPE_DOUBLE)
				.withFormatter(PERCENT_FORMATTER)
		...
		chunkSize.minus(nonWrittenRowsCount)
				.withFormatter(NUMBER_FORMATTER)
		...
		final CopperMeasure directMemory = perChunkAggregation(DatastoreConstants.CHUNK__OFF_HEAP_SIZE).sum()
		...
		directMemory.divide(directMemory.grandTotal())
				.withType(Types.TYPE_DOUBLE)
				.withFormatter(PERCENT_FORMATTER)
		...
		directMemory.divide(Copper.measure(USED_DIRECT))
				.withType(Types.TYPE_DOUBLE)
				.withFormatter(PERCENT_FORMATTER)
		...
		directMemory.divide(Copper.measure(MAX_DIRECT))
				.withType(Types.TYPE_DOUBLE)
				.withFormatter(PERCENT_FORMATTER)
				.as(MAX_MEMORY_RATIO)
				.withinFolder(CHUNK_MEMORY_FOLDER)
	...
	private void dictionaryMeasures(ICopperContext context) {
		...
		Copper.agg(chunkToDicoStore.field(DatastoreConstants.DICTIONARY_SIZE), SingleValueFunction.PLUGIN_KEY)
		...
		final CopperMeasure storeUnusedRows = Copper.measure(NON_WRITTEN_ROWS_COUNT)
				.per(
						Copper.hierarchy(OWNER_HIERARCHY).level(OWNER_HIERARCHY),
						Copper.hierarchy(FIELD_HIERARCHY).level(FIELD_HIERARCHY),
						Copper.hierarchy(PARTITION_HIERARCHY).level(PARTITION_HIERARCHY))
		...
		final CopperMeasure storeChunkSize = perChunkAggregation(DatastoreConstants.CHUNK__SIZE).max()
				.per(
						Copper.hierarchy(OWNER_HIERARCHY).level(OWNER_HIERARCHY),
						Copper.hierarchy(FIELD_HIERARCHY).level(FIELD_HIERARCHY),
						Copper.hierarchy(PARTITION_HIERARCHY).level(PARTITION_HIERARCHY),
	...
	private void vectorMeasures(ICopperContext context) {
		perChunkAggregation(DatastoreConstants.CHUNK__VECTOR_BLOCK_REF_COUNT).sum()
		...
		perChunkAggregation(DatastoreConstants.CHUNK__VECTOR_BLOCK_LENGTH).custom(SingleValueFunction.PLUGIN_KEY)
