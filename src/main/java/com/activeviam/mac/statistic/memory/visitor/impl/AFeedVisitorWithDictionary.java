...
public abstract class AFeedVisitorWithDictionary<R> extends AFeedVisitor<R> {
	...
	public AFeedVisitorWithDictionary(
			...
			IDatastoreSchemaMetadata storageMetadata,
			String dumpName) {
	...
	protected DictionaryAttributes processDictionaryStatistic(
		...
		if (!statistic.getName().equals(MemoryStatisticConstants.STAT_NAME_DICTIONARY_UNDERLYING)) {
			final IStatisticAttribute dictionaryIdAttribute = statistic.getAttribute(
					ATTR_NAME_DICTIONARY_ID);
			...
			if (!this.dictionaryAttributes.getDictionaryClass()
				...
				FeedVisitor.setTupleElement(
						tuple, format, DatastoreConstants.VERSION__EPOCH_ID, epochId);
