...
public class LevelStatisticVisitor extends AFeedVisitor<Void> {
	...
	public LevelStatisticVisitor(
		...
		this.directParentId = parent.pivot + "/" + parent.dimension + "/" + parent.hierarchy + "/" + parent.level;

		this.chunkIdCQ = this.transaction.getQueryRunner().createGetByKeyQuery(
	...
	private void processLevelMember(final IMemoryStatistic stat) {
		this.memberCount = stat.getAttribute(PivotMemoryStatisticConstants.ATTR_NAME_LEVEL_MEMBER_COUNT).asInt();
