...
class ContentServerToJsonUi {
	...
	static void writeSubTree(SnapshotContentTree node, Path currentPath) {
		for (Entry<String, ? extends IContentTree<BasicJsonContentEntry>> entry : node.getChildren()
				.entrySet()) {
	...
	static void createDirectory(Path path) {
		...
		if (!bookmarkFolder.exists()) {
			...
			if (!createdDirectories) {
				LOGGER.error(
						"Could not create export directories. Check path: " + path.getFileName());
			}
		}
	}
	...
	static void writeNode(String key, SnapshotContentTree node, Path folderName) {
		...
		try {
			...
			final Path fileName = folderName.resolve(
					encodeForFilesystems(key) + ContentServerConstants.Paths.JSON);
