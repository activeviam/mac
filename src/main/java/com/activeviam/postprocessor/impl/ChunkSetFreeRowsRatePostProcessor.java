...
@QuartetExtendedPluginValue(
    ...
    key = ChunkSetFreeRowsRatePostProcessor.PLUGIN_KEY)
public class ChunkSetFreeRowsRatePostProcessor
    extends ADynamicAggregationPostProcessor<Double, Double> {
   ...
   */
  public ChunkSetFreeRowsRatePostProcessor(
      String name, IPostProcessorCreationContext creationContext) {
  ...
  protected Double evaluateLeaf(ILocation leafLocation, Object[] underlyingMeasures) {
    if (
    /* Unlikely */ underlyingMeasures[1] == null) {
      return null;
    }

    int physicalSize = (int) underlyingMeasures[1];
    if (
    /* Unlikely */ physicalSize == 0) {
      return null;
    }
