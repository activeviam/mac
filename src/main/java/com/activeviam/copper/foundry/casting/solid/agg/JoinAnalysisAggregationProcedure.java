...
/**
 * The procedure for joining an isolated store to the cube's selection. The join may create analysis hierarchies. They
 * will then be handled by this {@link IAnalysisAggregationProcedure procedure}.
 *
 * @author ActiveViam
 */
...
public class JoinAnalysisAggregationProcedure extends AAnalysisAggregationProcedure<JoinContext> {
	...
	/**
	 * The mapping between the cube's selection via the cube levels (keys) and the isolated store via store field names
	 * (values).
	 */
	...
	/**
	 * Function that takes a cube filter as argument and that creates a {@link ICondition} from it. The condition will
	 * be used to create the appropriate {@link DatastorePrefetchRequest}.
	 */
	...
	/**
	 * Mapping from fields to corresponding level index in the hierarchical mapping. This is aligned with the the
	 * argument `fields`. See {@link JoinContext}.
	 */
	...
	public JoinAnalysisAggregationProcedure(
		...
		for (LevelCoordinate coord : this.mapping.keySet()) {
			final ILevel l = HierarchiesUtil.getLevel(context.getActivePivot(),
			                                          coord.parent.dimension,
			                                          coord.parent.hierarchy,
			                                          coord.level);
			this.mappingIndices.put(coord, new int[]{l.getHierarchyInfo().getOrdinal() - 1, l.getOrdinal()});
		}

		IHierarchicalMapping hierarchicalMapping = activePivot.getAggregateProvider().getHierarchicalMapping();
		if (!this.hierarchies.isEmpty()) {
			...
			final IHierarchicalMapping restrictedMapping = hierarchicalMapping.restrictMapping(restrictedLevels);
			// Create the fieldExpressionPerCoordinateList from the restricted mapping to ensure consistency of order for level coordinates
			for (int i = 0; i < restrictedMapping.getCoordinateCount(); i++) {
				...
				if (lInfo.getProperties() != null) {
					field = lInfo.getProperties().getProperty("fieldExpression") != null ? lInfo.getProperties().getProperty("fieldExpression") : lInfo.getName();
				} else {
					field = lInfo.getName();
				}
			...
			final ToDatastoreConditionFilterCompiler visitor = new ToDatastoreConditionFilterCompiler(
					fieldExpressionPerCoordinateList.toArray(new String[0]), false);
			this.filterCompiler = filter -> FilterVisitHelper.visit(filter,
			                                                        hierarchiesToVisit,
			                                                        restrictedMapping,
			                                                        visitor);
		} else {
		...
		final BiConsumer<LevelCoordinate, String> levelAndFieldConsumer = (level, field) -> {
			fieldsList.add(field);
			fieldMappingIdxsList.add(hierarchicalMapping.getCoordinate(this.mappingIndices.get(level)[0] + 1, this.mappingIndices.get(level)[1]));
		};
	...
	@Override
	public JoinContext createContext(IActivePivotVersion pivot,
		...
		return new JoinContext(
				targetScope,
				primitiveRetrievalScope,
				externalResult,
				this.fields,
				this.fieldMappingIdxs);
	}

	/**
	 * Iterates over each level declared in {@link #mappingIndices} and applies the given consumer on each of them.
	 *
	...
	@Override
	public boolean buildRequestScope(ILocationBuilder location,
		...
		if (joinMeasures.isEmpty()) {
			// A primitive measure is requested. Make sure the requested location is at a "correct" depth i.e every
			...
			// A requested join measure is provided by this procedure. Since in CoPPer, the creation of a join measure
			// is done with the SingleValueFunction, we need to make sure the requested location is a the correct depth
			// i.e every level of handled hierarchies is expressed
			forEachLevelInMapping(pivot, l -> {
				if (!LocationUtil.isAtOrBelowLevel(location, l)) {
					throw new QuartetRuntimeException(
							"One of the join measure " + joinMeasures + " was not request "
									+ "at the correct depth for the hierarchy " + l.getHierarchyInfo().getName()
	...
	/**
	 * Checks that at least one hierarchy that should be handled by this procedure is either expressed in the request
	 * location or a restriction exists on one of them.
	 *
	 * @param pivot the pivot
	 * @param location the requested location
	 * @param securityAndFilters cube filter grouping both security and filters restrictions
	 * @return true if at least one hierarchy that should be handled by this procedure is either expressed in the request
	 * location or a restriction exists on one of them, false otherwise.
	 */
	protected boolean handledHierarchiesAreExpressedOrRestricted(IActivePivotVersion pivot, ILocationBuilder location, ICubeFilter securityAndFilters) {
		if(getHandledHierarchies().isEmpty()) {
			// If no hierarchy, always true!
			return true;
		}

		final TIntObjectMap<HasAnalysisLevelCoordinatesResult> expressedHierarchies = LocationUtil.collectExpressedAnalysisHierarchies(location,
	...
	@Override
	public void mapToFullLocation(JoinContext context,
	...
	@Override
	public DatastorePrefetchRequest getDatastorePrefetch(ILocation prefetchLocation, ICubeFilter filter) {
		...
		return new DatastorePrefetchRequest(this.store,
