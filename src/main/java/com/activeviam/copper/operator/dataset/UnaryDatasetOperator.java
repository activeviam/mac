...
public abstract class UnaryDatasetOperator implements DatasetOperator {
	...
	/**
	 * Gives the list of non constant {@link CoreColumn#hasConstantValues() non constant} columns from our
	 * {@link #operand}.
	 *
	 * @return The list of non constant {@link CoreColumn#hasConstantValues() non constant} columns from
	 * our {@link #operand}.
	 */
	...
	/**
	 * Extract the values supplier from a column contained in the dataset created by a {@link DatasetOperator}.
	 *
	 * @param operator The operator that created the dataset. This operator might not be a {@link UnaryDatasetOperator}
	 *        (hence this static method).
	 * @param createdColumn The created column.
	 *
	 * @return The values supplier.
	 */
	public static DistinctValuesSupplier getMembersSupplierFor(DatasetOperator operator, CoreColumn createdColumn) {
		// First see if the column has a distinct member supplier
		// This could be the case if user explicitly specifies the column's members using .withMemberList().
		// In which case, we return the column's supplier. Otherwise, get the output supplier from the operator.
	 ...
	 */
	public static void throwOnNameConflict(ImmutableCollection<CoreColumn> among) throws UserErrorException {
		Map<String, CoreColumn> current = new HashMap<>();
		among.forEach(c -> {
			String name = c.name();
			CoreColumn previous = current.put(name, c);
			if (previous != null) {
				boolean toStringAreUseless = String.valueOf(c).equals(name) && String.valueOf(previous).equals(name);
				String detail = toStringAreUseless ? "" : ": "+ previous + " and " + c;
				throw new UserErrorException("Two columns have the name '" + c.name() + "'" + detail + ".");
			}
		});
	}
}
