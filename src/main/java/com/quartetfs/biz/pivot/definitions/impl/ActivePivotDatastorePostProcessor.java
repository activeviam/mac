...
/**
 * A {@link IDatastoreSchemaDescriptionPostProcessor} that propagates ActivePivot
 * constraints to the underlying datastore configuration.
 * <ul>
...
public class ActivePivotDatastorePostProcessor implements IDatastoreSchemaDescriptionPostProcessor {
	 ...
	 */
	public ActivePivotDatastorePostProcessor(Collection<IActivePivotSchemaDescription> schemasDesc, IDatastoreSchemaDescriptionPostProcessor... postProcessors) {
	...
	public IDatastoreSchemaDescription process(final IDatastoreSchemaDescription original) {
				...
				if (fields.contains(storeName+ SEP + field.getName())){
					...
					if (type == null) {
						throw new ConfigurationException(
								"The field named " + field.getName() + " in store " + originalStore.getName() + " is of Unknown type.",
								"Please make sure that the type defined in the field description is a valid one.");
					}
					if (type.isArray()) {
						throw new ConfigurationException(
								"The field named " + field.getName() + " in store " + originalStore.getName() + " is of type Vector and " +
		...
		IDatastoreSchemaDescriptionPostProcessor nonNullable = new NonNullableFieldsPostProcessor(fields) {
			...
			@Override
			protected void nonNullableFieldWithNullDefaultValueHandler(IStoreDescription storeDescription, IFieldDescription fieldDescription) {
				throw new ConfigurationException(
		...
		IDatastoreSchemaDescriptionPostProcessor dictionarize = new DictionarizeFieldsPostProcessor(fields);
	...
	/**
	 * Create the list of fields to dictionarize in the Datastore based on a
	 * selection and the list of the fields names of this selection that need to
	 * be dictionarized on ActivePivot side.
	 *
	 * @param selection the Datastore field selection.
	 * @param fieldsToDictionarize the list of field names in this selection that
	 * 		  need to be dictionarized.
	 *
	 * @return the list of fields to dictionarize.
	 */
	protected static Collection<String> extractFields(
			ISelectionDescription selection,
			Collection<String> fieldsToDictionarize) {
	...
	public static void addFieldsFromCubes(
			...
			IActivePivotSchemaDescription schemaDesc) {
		for (IActivePivotInstanceDescription apid : schemaDesc.getActivePivotInstanceDescriptions()) {
			for (IAxisDimensionDescription add : apid.getActivePivotDescription()
					.getAxisDimensions()
					.getValues()) {
	...
	/**
	 * Creates a schema post processor that applies ActivePivot requirements
	 * to an existing datastore description.
	 *
	...
	public static ActivePivotDatastorePostProcessor createFrom(IActivePivotManagerDescription desc) {
		List<IActivePivotSchemaDescription> schemas = desc
				.getSchemas()
				.stream()
				.map(IActivePivotSchemaInstanceDescription::getActivePivotSchemaDescription)
				.collect(Collectors.toList());
		return new ActivePivotDatastorePostProcessor(schemas,
				numaSelectorPostProcessor(schemas));
	}
	 ...
	 * @param incrementedExpressions collection to complete with field expresssions incremented by this call
			...
			for (IAxisHierarchyDescription hier : di.getHierarchies()) {
				...
				if (scope == null) {
					...
					if (dimScope == null) {
						...
						final Map<String, String> noDimDefScope = scope.get(PartialProviderDefinition.NO_DIMENSION_DEFINED);
	 ...
	 * @param numBitmapByDatastoreField for each base store's field, the number of bitmap providers that contain it
	 */
	public static void countBitmap(IActivePivotSchemaDescription schema, TObjectIntMap<String> numBitmapByDatastoreField) {
		...
		for (IActivePivotInstanceDescription instance : schema.getActivePivotInstanceDescriptions()) {
			...
			final boolean isBitmap = globalProvider.getPluginKey().equals(BitmapAggregateProviderBase.PLUGIN_TYPE);
			final boolean isMultiple = globalProvider.getPluginKey().equals(GlobalMultipleAggregateProviderBase.PLUGIN_TYPE);
			final boolean withBitmapUnderlying = (globalProvider.getProperties() == null) ? true : globalProvider.getProperties()
					.getProperty(UNDERLYINGPROVIDER_PLUGINKEY, BitmapAggregateProviderBase.PLUGIN_TYPE)
			...
			if (globalProvider.getPartialProviders() != null) {
				for (IPartialProviderDefinition partial : globalProvider.getPartialProviders()) {
					if (partial.getPluginKey().equals(BitmapAggregateProviderBase.PLUGIN_TYPE)) {
						incrementExpressionCounter(fieldToExpression, numBitmapByDatastoreField, partial, pivot.getAxisDimensions());
					}
				}
			}
		}
	...
	/**
	 * Creates a {@link WeightMaximizingNumaSeletorPostProcessor} that uses "the number of bitmap providers that contain each datastore field"
	 * as criteria of choosing {@link INumaNodeSelector numa selector} field for each store.
	 *
	 * @param schemas list of schemas to consider
	 * @return the numa post processor
	 */
	public static WeightMaximizingNumaSeletorPostProcessor numaSelectorPostProcessor(final Collection<? extends IActivePivotSchemaDescription> schemas) {
	 ...
	 */
	protected static class JoinProcedurePostProcessor implements
		...
		public IDatastoreSchemaDescription process(
			...
			List<IReferenceDescription> refs = new ArrayList<>(
					datastoreSchemaDescription.getSameDictionaryDescriptions());
			for (IActivePivotInstanceDescription cubeDescription : this.schemaDescription
					.getActivePivotInstanceDescriptions()) {
				refs.addAll(extractJoinAggregationProcedureReferences(datastoreSchemaDescription,
						this.schemaDescription, cubeDescription.getActivePivotDescription()));
			}
			return refs.isEmpty()
					? datastoreSchemaDescription
					: new DatastoreSchemaDescription(datastoreSchemaDescription.getStoreDescriptions(),
							datastoreSchemaDescription.getReferenceDescriptions(), refs);
		}

		/**
		 * Builds the references coming from join made in Copper. These references are only used for
		 * sharing dictionaries when possible. See {@link IDatastoreSchemaDescription#getSameDictionaryDescriptions()}.
		 *
		 ...
		 *                                   IAnalysisAggregationProcedureDescription} that describe the
		 *                                   joins between the cube and other (isolated) stores)
      ...
      IUserDatastoreSchemaDescription userDatastoreSchemaDescription =
          UserDatastoreSchemaDescription.createFromSchemaDescription(datastoreSchemaDescription);
			...
			for (IAnalysisAggregationProcedureDescription procedure : procedures) {
				if (AJoinAnalysisAggregationProcedure.JOIN_KEYS.contains(procedure.getPluginKey())) {
					...
					Map<LevelIdentifier, String> mapping = (Map<LevelIdentifier, String>) procedure
							.getProperties().get(AJoinAnalysisAggregationProcedure.MAPPING);
					String store = (String) procedure.getProperties()
							.get(AJoinAnalysisAggregationProcedure.STORE);

					Map<LevelIdentifier, IPair<String, String>> info = getStoreAndFieldByLevel(
							mapping.keySet(),
					...
					for (Map.Entry<LevelIdentifier, IPair<String, String>> entry : info.entrySet()) {
						...
						final Pair<String, String> processedFromPath = processPath(fromField, fromStore,
								datastoreSchemaDescription);
						...
						final Pair<String, String> processedToPath = processPath(toField, toStore,
								datastoreSchemaDescription);
						...
						result.add(new ReferenceDescription(
								fromStore,
								toStore,
								referenceName,
		 ...
		 */
		protected static Pair<String, String> processPath(final String field,
				final String store,
				IDatastoreSchemaDescription datastoreSchemaDescription) {
			...
			if (splitFromField.length > 1) {
				...
				List<IStoreDescription> startStores = datastoreSchemaDescription.getStoreDescriptions()
						.stream().filter(p -> p.getName().equals(finalFromStore)).collect(
								Collectors.toList());
				assert startStores.size() == 1 : "There should be exactly one store with the name"
						+ finalFromStore + " in the datastore";
				...
				for (int i = 0; i < splitFromField.length - 1; i++) {
					...
					List<? extends IReferenceDescription> validRefsFromCurrent = datastoreSchemaDescription
							.getReferenceDescriptions().stream()
							.filter(r -> r.getOwnerStore().equals(finalCurrentStore.getName())
									&& r.getName().equals(splitFromField[0]))
							.collect(Collectors.toList());
					assert validRefsFromCurrent.size()
							== 1 : "There should be exactly one valid reference on the path";
					currentStore = datastoreSchemaDescription
							.getStoreDescriptions().stream()
							...
							if (hier.getName().equals(coordinate.parent.hierarchy)) {
								for (IAxisLevelDescription lvl : hier.getLevels()) {
									if (lvl.getLevelName().equals(coordinate.level)) {
										...
										for (ISelectionField field : datastoreSelection.getSelectionFields()
												.getValues()) {
											if (field.getName().equals(lvl.getPropertyName())) {
												String[] path = DatastoreSchemaDescriptionUtil
														.splitPath(field.getExpression());
												IUserDatastoreSchemaDescription.IUserStoreDescription storeDescription =
														userDatastoreSchemaDescription.getStoreDescriptionAlongPath(
																datastoreSelection.getBaseStore(),
																path);
												fieldInfoByLevelCoord.put(coordinate,
														new Pair<>(storeDescription.getStoreName(),
																path[path.length - 1]));
												continue levelLoop;
											}
										}
